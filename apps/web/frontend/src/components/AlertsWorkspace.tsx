...
export function AlertsWorkspace() {
  ...
  return (
    <div className="alerts-workspace">
      ...
      <Modal open={dialogOpen} onCancel={() => setDialogOpen(false)} footer={null} width={760} centered destroyOnHidden className="alerts-edit-modal" title={<div className="quote-dialog-title alerts-dialog-title"><div><strong>{editing ? '编辑事件' : '添加宏观事件'}</strong><span>{editing?.name || '交易前风险日历'}</span></div><small>EVENT SCHEDULE</small></div>}>
      ...
      <Modal open={settingsOpen} onCancel={() => setSettingsOpen(false)} footer={null} width={560} centered destroyOnHidden className="alerts-settings-modal" title={<div className="quote-dialog-title alerts-dialog-title"><div><strong>提醒规则</strong><span>交易前风险窗口</span></div><small>ALERT POLICY</small></div>}>
