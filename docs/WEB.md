...
| 交易系统 | 浏览器本地 `localStorage`，保存账户、纪律清单、交易卡、复盘和导入导出 JSON |
| 自动化任务 | `automation_tasks` 表，保存 Cron、任务类型、payload、Webhook 关联 |

## 交易系统

交易系统入口位于顶部工作区 `交易系统`。第一版不接实时行情接口，先用于个人交易计划、风控计算和复盘记录。
...
页面会自动计算持仓市值、仓位占比、浮动盈亏、最大计划亏损和账户风险。数据保存在当前浏览器本地，可用 `导出` 备份，也可用 `导入` 恢复。完整规则见 [TRADING_SYSTEM_V1.md](TRADING_SYSTEM_V1.md)。
