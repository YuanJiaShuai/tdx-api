...
func NewCodes(args ...any) (*Codes, error) {
	var (
		c  *Client
		db *xorm.Engine
	)
	if len(args) == 0 {
		...
		return NewCodesSqlite(c)
	}
	if len(args) != 2 {
		return nil, errors.New("NewCodes expects no args or (*Client, *xorm.Engine)")
	}
	var ok bool
	c, ok = args[0].(*Client)
	if !ok {
		return nil, errors.New("NewCodes first arg must be *Client")
	}
	db, ok = args[1].(*xorm.Engine)
	if !ok {
		return nil, errors.New("NewCodes second arg must be *xorm.Engine")
	}
...
func (this *Codes) GetIndexes(limits ...int) CodeModels {
	...
	ls := CodeModels(nil)
	for _, m := range this.list {
		if protocol.IsIndex(m.FullCode()) {
			ls = append(ls, m)
		}
		if limit > 0 && len(ls) >= limit {
			break
