...
// skipWithoutLocalData 本地通达信安装目录不存在时跳过(测试依赖 Windows 安装的通达信数据文件)
func skipWithoutLocalData(t *testing.T) {
	t.Helper()
	if _, err := os.Stat(`D:\软件\通达信\`); err != nil {
		t.Skip("本地通达信数据目录不存在,跳过")
	}
}

func TestReadDay(t *testing.T) {
	skipWithoutLocalData(t)
